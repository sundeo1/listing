...
const RealEstate = () => {
    ...
    const fetchData = async () => {
      try {
        const res = await axios.get(
          'http://localhost:8000/api/listings/real-estate?page=1'
        );
  ...
  const visitPage = (page) => {
    axios
      .get(`${process.env.REACT_APP_API_URL}/api/listings/real-estate?page=${page}`)
      .then((res) => {
        setListings(res.data.results);
        setPrevious(res.data.previous);
        setNext(res.data.next);
  ...
  return (
    <main className='listings'>
      <Helmet>
        <title>Listing App - RealEstate</title>
        <meta name='description' content='RealEstate page' />
      </Helmet>
