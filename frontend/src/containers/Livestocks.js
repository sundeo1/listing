...
const Livestocks = () => {
    ...
    const fetchData = async () => {
      try {
        const res = await axios.get(
          'http://localhost:8000/api/listings/livestock?page=1'
        );
  ...
  const visitPage = (page) => {
    axios
      .get(`${process.env.REACT_APP_API_URL}/api/listings/livestock?page=${page}`)
      .then((res) => {
        setListings(res.data.results);
        setPrevious(res.data.previous);
        setNext(res.data.next);
  ...
  return (
    <main className='listings'>
      <Helmet>
        <title>Listing App - Livestocks</title>
        <meta name='description' content='Livestocks page' />
      </Helmet>
